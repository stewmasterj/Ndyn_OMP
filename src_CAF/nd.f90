...
!}}}**************************************************************************80
  ...
  if (THIS_IMAGE().eq.1) then
     write(6,'(a)') 
     write(6,'(a)') '#  N D Y N'
     write(6,'(a)') '#  FORTRAN90/CAF/OpenMP version 1.20.12.22'
     write(6,'(a)') 
     ...
     write(6,'(a,i4)') '#  Number of images specified:   ', NUM_IMAGES()
     write(6,'(a)') 
     call timestamp ( )
  endif
  ...
  CALL CO_MAX( tottime, RESULT_IMAGE=1 )
  if (THIS_IMAGE().eq.1) then
     call timestamp ( )
...
! Initializing cell linked list parameters
  if (.not.allocated(pos))     call read_model
  if (skin < 0.d0)             skin = rcut*0.2d0
  ...
  if (myImg==1) write(6,'(A,3i4)') "# Number of link cell: ",MCELL

  if (NeiMax.eq.0) then
    ! approximate max neighbours +10% in cutoff radius
    nn = (np)
    call CO_SUM(nn, RESULT_IMAGE=1) ! get total system number
    NeiMax = int(4./3.*PI*((rcut+skin)*1.1)**3*nn/(box(1)*box(2)*box(3)))
    if (myImg==1) write(6,'(A,i4)') "# Estimating max number of neighbours: ",NeiMax
  endif

  if (allocated(HEAD)) then
    deallocate( HEAD, VNL, List, dispList, ftmp )
  endif
  allocate( HEAD(NCELL), VNL(1:NeiMax+1,1:ourNp), List(ourNp), dispList(3,ourNp), &
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!80
  ...
  call getGhostInit ! share the newly integrated position data.
...
!
  ...
  write(ct,'(i4.4)') myImg
  open(11+myImg,file='out.'//trim(ModelOutType)//"_"//ct//".dd",position="append")
  call write_model(11+myImg)
  wtime = omp_get_wtime ( )
...
!    Update positions, velocities, accelerations.
  do tt = 1, step_num
    ...
    ! Compute forces etc
    call integrate1
    call getGhostPositions ! share the newly integrated position data.
    if     (     asympots.and.     springpots) then ! GP-PD
      call compute3(calcVirial,calcPot)
    elseif (.NOT.asympots.and.     springpots) then ! PD
      call compute2(calcVirial,calcPot)
    elseif (     asympots.and..NOT.springpots) then ! GP-MD
    ...
    if (mod(step,ConfOuts) == 0)              call write_model(11+myImg)
...
!  Terminate.
!
  call flush(0);   call flush(6);   call flush(11+myImg)
  close(11+myImg)

  wtime = omp_get_wtime ( ) - wtime
  CALL CO_MAX( wtime, RESULT_IMAGE=1 )
  if (myImg==1) then
...
subroutine getGhostInit 
!{{{**************************************************************************80
! communicate the positions on the neighbouring image here as a ghost layer
! Wed Oct 10, 2020
!
  use commMod
  use mostCommon, only : pos, typ
  implicit none
...
do i = 1, com%Nget
  ...
  typ( my(1):my(2) ) = typ( ur(1):ur(2) )[urID]
enddo
SYNC ALL ! needed now, since bulding the VNL is next

end subroutine getGhostInit !}}}
subroutine getGhostPositions 
!{{{**************************************************************************80
! communicate the positions on the neighbouring image here as a ghost layer
! Sat Sep 12 14:31:08 EDT 2020
!
  use commMod
  use mostCommon, only : pos
  implicit none
  integer :: i, my(2), ur(2), urID
...
!
  ...
  integer(4) :: i, jn, j, k, typi, PTij, istart, iend, tid, tido, n
...
!$omp shared ( virial, pene, ftmp, vtmp, ptmp, CU, n, myNp, ourNp ) &
!$omp private ( d2, i, j, k, jn, rij, dp, df, kt, sr, weight, rk, tmpi) &
!$omp private ( typi, tid, istart, iend, PTij, tido )
tid=omp_get_thread_num()

! split threads for bulk particles in this image
n      = com%bulkrange(2) -com%bulkrange(1)
istart = int(n/thread_num)*tid     +com%bulkrange(1) 
iend   = int(n/thread_num)*(tid+1) +com%bulkrange(1) -1
if (iend<com%bulkrange(2).and.tid.eq.thread_num-1) iend = com%bulkrange(2)
...
!  Attribute half of the potential energy to particle I and J.
      ...
      if (CU) then
        ...
        !pene(i) = pene(i) + dp ! contribution j to i and i to j
        ptmp(tid+1) = ptmp(tid+1) + dp ! full bond contribution to total
      endif
      if (CV) then
...
! this must NOT be inside of an OMP parallel structure
SYNC ALL ! ensure all ghost neigbours got recieved
...
! split threads for boundary particles in this image
n      = myNp -com%bulkrange(2) -1 
istart = int(n/thread_num)*tid     +com%bulkrange(2) +1
iend   = int(n/thread_num)*(tid+1) +com%bulkrange(2) 
if (iend<myNp.and.tid.eq.thread_num-1) iend = myNp
...
!  Attribute half of the potential energy to particle I and J.
      ...
      ftmp(2,tido) = ftmp(2,tido) + rij(2)*df
      ftmp(3,tido) = ftmp(3,tido) + rij(3)*df
      if (j <= MyNp) then ! the neighbour is on this image, not a ghost
         ftmp(1,j+tid*np) = ftmp(1,j+tid*np) - rij(1)*df
         ftmp(2,j+tid*np) = ftmp(2,j+tid*np) - rij(2)*df
         ftmp(3,j+tid*np) = ftmp(3,j+tid*np) - rij(3)*df
      endif

      if (CU) then ! accumulate total system potential energy
        dp = weight* PPairTable(k+1,PTij) + &
           &  (1.d0-weight)* PPairTable(k,PTij)
        !pene(i) = pene(i) + dp ! full bond contribution
        if (j <= MyNp) then ! count this bond's energy fully
          ptmp(tid+1) = ptmp(tid+1) + dp
        else  ! count only half this energy, other atom on other image
          ptmp(tid+1) = ptmp(tid+1) + dp*0.5D+00
        endif
      endif
      if (CV) then ! accumulate the total system virial stress
        if (j > MyNp) df = df*0.5D+00 ! only count half
        vtmp(1,tid+1) = vtmp(1,tid+1) + rij(1)*rij(1)*df
        vtmp(2,tid+1) = vtmp(2,tid+1) + rij(2)*rij(2)*df
        vtmp(3,tid+1) = vtmp(3,tid+1) + rij(3)*rij(3)*df
        vtmp(4,tid+1) = vtmp(4,tid+1) + rij(1)*rij(2)*df
...
!
  ...
  real(8) :: d2, rij(3), dp, df, kt, tmpi(3), sr(3), weight, ptmp(8), vtmp(6,8)
  real(4) :: rk, vPTdr2(NPairs)
  integer(4) :: i, jn, j, k, typi, PTij, PTji, istart, iend, tid, tido, mxPTij, n
  logical, intent(in) :: CV, CU
...
! this must NOT be inside of an OMP parallel structure
SYNC ALL ! ensure all ghost neighbours got recieved
...
!      if (k < 1) k = 1                ! unlikely but just to protect
      ...
      if (PTij.gt.0 .and. j <= MyNp) then ! neigh on this IMG not ghost
        ftmp(1,j+tid*np) = ftmp(1,j+tid*np) - rij(1)*df
        ftmp(2,j+tid*np) = ftmp(2,j+tid*np) - rij(2)*df
        ftmp(3,j+tid*np) = ftmp(3,j+tid*np) - rij(3)*df
      endif
...
! this must NOT be inside of an OMP parallel strucure
SYNC ALL ! ensure all ghost neighbours got recieved
...
!
  ...
  real(8) :: rij0(3), d20, sij, rc2, Vi, r, r0
  real(4) :: rk, vPTdr2(NPairs)
  integer(4) :: i, jn, j, k, typi, PTij, PTji, mxPTij, istart, iend, tid, tido, n
  logical, intent(in) :: CV, CU
...
! this must NOT be inside of an OP parallel structure
SYNC ALL ! ensure all ghost neighbours got recieved
...
!
  ...
  implicit none
  integer ( kind = 8 ) j
  write(6,'(A,3f10.4)') "# RandomPositions: using system box: ",box(1:3) 
!
...
!$omp parallel workshare &
!$omp shared ( HEAD, List, NCELL, ourNp )
  HEAD(1:NCELL) = 0
  List(1:ourNp) = 0
!$omp end parallel workshare

!omp parallel do &
!omp shared ( pos, MCELL, ourNp, NCELL) &
...
!logical :: outs
  ...
  !dreiPnd = 14 !(3**3-1)/2+1 for non MPI newton pair
  dreiPnd = 27 !3**3 ! also add the j>i thing in the second loop
!omp parallel do &
...
! save the current system positions to be used as initial positions used for spring potentials
  ...
  call getGhostPositions ! share the neighbouring images' positions
  SYNC ALL ! make sure communication is complete before copying.
