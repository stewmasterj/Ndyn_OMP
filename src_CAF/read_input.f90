...
character(len=80) :: tmp, word, var, fil
real(4) :: r4tmp
...
! Particles
  ...
  case ("Model"); Model=trim(s_get_word(2,tmp))
       ...
       call read_model
  case ("ComFile"); var = trim(s_get_word(2,tmp))
       call readCOMfile( trim(var) )
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
subroutine readCOMfile( fil )
! {{{
! read the communications file preoduced from 'domdec'
use commMod
use mostCommon
implicit none
character(*) :: fil
...
read(10,*) com%maxOurNp ! max image ourNp for allocation of coarray
read(10,*) box ! global boxsize

do i = 1, Nim ! images are writen in order, so no need to check
  if (i==myImg) then ! read the section
    read(10,*) j, myNp, ourNp, com%NNcells
...
! let's call this
np = myNp
! allocation before reading model file
!allocate( pos(3,com%maxOurNpt)[*] ) ! allocate
!allocate( pene(OurNp) )
...
! no header for the domdec model outputs, all data in the dd.com file
...
! allocate all the properties
allocate( pos(3,com%maxOurNp)[*] ) ! for coarray, only thing that's shared
allocate( typ(com%maxOurNp)[*] ) ! for coarray, only thing that's shared
allocate( vel(3,np), acc(3,np), force(3,np), pene(np), &
  &  grp(np), groupName(grpBuff), hold(np), holdName(holdBuff), &
  &  bcTyp(holdBuff), bcParm(3,holdBuff), bcDir(3,holdBuff) )
...
write(6,'(A,i8,A,i4)') "#  read_model: successfully read ",np, &
   &  " model nodes on Image:",myImg
i = np ! temp integer
SYNC ALL ! wait for each image to finish reading model file
call co_sum( i, RESULT_IMAGE=1 ) ! send total sum to img=1
if (myImg==1) write(6,'(A,i10)') "#  read_model: total system node count:",i

Return

800 continue
write(0,*) "read_model: ERROR: on line: ",ln, " image:",myImg
call flush(0); STOP 
...
select case (trim(ModelOutType))
  case("XYZ",'xyz')
    ...
    write(FD,'(A)')
    do i = 1, np
      write(FD,'(A4,x,7EN14.4,x,i3)') typeName(typ(i)), pos(1:3,i), vel(1:3,i), pene(i), VNL(1,i)-1
    enddo
  case("dump","trj")
...
! write some periodic system statistics
  ...
  r(1) = potential; CALL CO_SUM( r(1), RESULT_IMAGE=1 )
  r(2) = kinetic;   CALL CO_SUM( r(2), RESULT_IMAGE=1 )
  me = e0;          CALL CO_SUM( me,   RESULT_IMAGE=1 )
  r(3) = ( r(1) + r(2) - me ) / me
  v = box(1)*box(2)*box(3)
  s = virial(1:3);  CALL CO_SUM( s,    RESULT_IMAGE=1 )
  r(4) = sum(s)/(v*3.0)
  if (THIS_IMAGE().eq.1) then
     write(6, '(x,i8,5(x,g14.6))' )  step, r(1:4), v 
     call flush(6)
  endif
